...
class BaseEval:
    ...
    def __init__(self, args: Arguments, corpus: Dataset, **kwargs):
        ...
        for doc_id, task_name in zip(self.corpus['id'], self.corpus['task_name']):
            self.task_name_to_doc_ids[task_name].add(str(doc_id))
    ...
    def get_prompt_by_doc_ids(self, doc_ids: List[str]) -> str:
        return '\n\n'.join([self.corpus[int(doc_id)]['contents'] if not self.args.keyword_augment else self.corpus[int(doc_id)]['original_contents'] for doc_id in doc_ids])
