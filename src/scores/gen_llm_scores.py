...
def _select_indices(dataset: Dataset) -> List[int]:
    ...
    while left < right:
        middle = (left + right) // 2
        total_examples = sum([min(middle, v)
                             for v in num_examples_per_task.values()])
...
def _worker_gen_llm_scores():
    ...
    assert os.path.exists(
        data_path), 'Data file {} does not exist.'.format(data_path)
    ...
    if args.dry_run:
        ...
        model_dependent_seed: int = args.seed + \
            hash(args.llm_model_name_or_path) % 10 ** 6
    ...
    dataset = dataset.shard(num_shards=args.world_size,
                            index=gpu_idx, contiguous=True)
    logger.info('Worker {} needs to process {} examples'.format(
        gpu_idx, len(dataset)))
    ...
    def _map_func(examples: Dict[str, List]) -> Dict[str, List]:
        ...
        for instruction, query, doc_ids, answers in zip(examples["instruction"], examples['query'], examples['doc_ids'], examples['answers']):
            answer = answers[0]
            assert all(int(doc_id) >= 0 for doc_id in doc_ids)
            for doc_id in doc_ids:
                input_texts.append('{}\n\n{}\n\n{}'.format(
                    instruction, corpus[int(doc_id)]['contents'], query.strip()))
                output_texts.append(answer)

        llm_scores: List[float] = llm.batch_score(
            input_texts=input_texts, output_texts=output_texts, delimiter=" ")
    ...
    logger.info('Worker {} saved {} examples to {}'.format(
        gpu_idx, len(dataset), out_path))
...
def main():
    ...
    out_path: str = '{}/{}_{}.jsonl.gz'.format(
        args.data_dir, parse_model_id(
            args.llm_model_name_or_path), args.search_split
    )
    ...
    wait_until_all_files_show_up([_get_shard_out_path(
        gpu_idx) for gpu_idx in range(args.world_size)])
