...
def _worker_gen_teacher_score():
    ...
    # 这里是枚举每个 query 的 doc, 然后组成 (doc, query) 对
    qid_pids = []
    for ex in tqdm.tqdm(dataset, desc='get qid-pid pairs', mininterval=3):
        for doc_id in ex['doc_ids']:
            qid_pids.append((ex['query_id'], doc_id, ex['query'], ex['answers']))
    ...
    if args.keyword_augment:
        logger.info("Using keyword to augment retrieval.")
        tokenizer.add_tokens(["<Keyword>", "</Keyword>"])
