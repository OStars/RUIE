...
class CrossEncoderDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, input_files: List[str], args: Arguments,
        ...
        corpus_path = os.path.join(os.path.dirname(
            self.input_files[0]), 'passages.jsonl.gz')
        self.corpus: Dataset = load_dataset(
            'json', data_files=corpus_path, split='train')

        self.dataset: Dataset = load_dataset(
            'json', data_files=self.input_files, split='train')
        ...
        if self.args.max_train_samples is not None and not self.args.dry_run:
            self.dataset = self.dataset.select(
                range(self.args.max_train_samples))
        # Log a few random samples from the training set:
        for index in random.sample(range(len(self.dataset)), 1):
            logger.info(
                f"Sample {index} of the training set: {self.dataset[index]}.")
    ...
    def _transform_func(self, examples: Dict[str, List]) -> Dict[str, List]:
        current_epoch = int(
            self.trainer.state.epoch or 0) if self.trainer is not None else 0
        ...
        for idx, doc_id in enumerate(input_doc_ids):
            ...
            answers = examples['answers'][q_idx]
            current_query +=  " " + random.choice(answers)
        ...
        for k in batch_dict:
            packed_batch_dict[k] = []
            assert len(examples['query']) * \
                self.args.train_n_passages == len(batch_dict[k])
            for idx in range(len(examples['query'])):
                start = idx * self.args.train_n_passages
                packed_batch_dict[k].append(
                    batch_dict[k][start:(start + self.args.train_n_passages)])
