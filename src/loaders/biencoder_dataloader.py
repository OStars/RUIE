...
class BiencoderDataset(torch.utils.data.Dataset):
    def __init__(self, args: Arguments,
        ...
        if not self.args.dry_run and self.args.max_train_samples is not None:
    ...
    def _transform_func(self, examples: Dict[str, List]) -> Dict[str, List]:
        ...
        queries = examples['query'][:] if not self.args.keyword_augment else examples["original_query"]
        ...
        if self.args.add_qd_prompt:
            ...
            input_docs = ['passage: ' + d for d in input_docs]
